...
from utils import otc2otf, otf2otc, ttx_extract_name, ttx_merge, run_powershell_command, info, warning
...
class TTCConverter(BaseConverter):
    ...
    def prepare_resource(self, mapper: MapperConfig):
        ...
        2. ttx 提取 name 表
        """
        backup_file = os.path.join(mapper.backup_dir, os.path.basename(mapper.source_file))
    ...
    def convert(self):
        ...
        for mapper in self.mappers:
            ...
            if not mapper.output_file:
                ...
                mapper.output_file = os.path.join(target_dir, os.path.basename(mapper.source_file))
